...
The teams getting the most value from Agentforce Vibes are not the ones typing the fastest prompts. They are the ones who have learned to guide, constrain, and verify the AI's work with intention.
...
- **Features touching multiple parts of your codebase.** Deep planning maps dependencies and prevents surprise edits when changes span services, UI, and tests.
...
Every Salesforce org has conventions. AI tools do not know them by default.

- Write persistent rules for your standards. Use rules to lock in conventions for structure, naming, testing expectations, libraries, and error handling.
...
- Create a project definition document. A Product Requirements Document or instructions file describing the app's purpose, features, and tech stack serves as a north star for both you and the AI.
- Define coding guidelines in your rules. Include naming conventions, preferred frameworks or libraries, code style, architectural patterns, and other standards.
- Keep rule files focused and concise. Under 500 lines is a good target. Give each rule a clear name and description so the AI can apply them appropriately.
...
- Disable rules that do not apply. Irrelevant rules add noise to the AI's context and can lead to confusion or unnecessary guidance. If you are not building mobile functionality, disable the mobile rules.
...
- Commit your code before large AI actions. This gives you a clean state to compare against or revert to if needed.
...
- Configure allowed actions explicitly. Provide an allow list of safe commands (build, test, lint) and a deny list for destructive actions.
...
- End every task with proof. Ask Agentforce Vibes to run unit tests, lint, typecheck, and build, then fix failures until clean.
...
- Adopt a test-driven mentality. Ask Agentforce Vibes to generate unit tests first, then implement the code until all tests pass.
...
- Let the AI act, but set boundaries. Configure allowed actions and validate the approach before broad changes.
- Monitor multi-file changes closely. Watch the agent's output as it goes through each step.
- If Agentforce Vibes starts altering a file you did not intend to touch, stop and refine your prompt to constrain the scope.
- Break down large refactors into stages rather than attempting everything at once.
- For very extensive edits, use multiple prompts. For example, "Refactor module X (first half)" then "Now refactor module X (second half)."

## Reviewing AI Output

Treat Agentforce Vibes as a highly productive assistant, not an infallible one.
...
- Verify adherence to coding standards. Run linters and formatters. Prompt Agentforce Vibes to refactor if the output could be cleaner.
- If something looks off, ask Agentforce Vibes to explain how the code works to verify its reasoning.
- Generate documentation. Ask Agentforce Vibes to write a README section or docstrings explaining how a feature works. If the documentation does not match your understanding, that is a red flag.
...
Agentforce Vibes is a tool to augment development, not replace thinking.
...
- Do not be afraid to intervene when necessary. You remain the senior developer in the loop.
- If Agentforce Vibes's output is drifting from your goals, step back and re-evaluate the instructions you provided.
